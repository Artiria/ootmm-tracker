...
real seeds works too), per ROM:
  ...
  placement   each check's item against the spoiler log next to the ROM, and
              in a multiworld whose item it is. The names are compared with
              `placement.same_item`, which grew here and now lives in the
              tracker because the overlay vets a spoiler with the very same
              judgement: the spellings the two sides are known to differ on (a
              trap's cloak, `Bottle of X`, the shared/split variants) are
              normalised away and a genuinely different item still shows. A location the seed did not shuffle (a cow with
...
# same_item and its tables moved into the tracker itself on 27 ago 2026:
# the overlay needs the same judgement to vet a spoiler (placement.py)
from placement import same_item      # noqa: E402,F401
...
def item_unverifiable(spoiler_item, rom_item):
    ...
    a = placement.bare_item(spoiler_item)
